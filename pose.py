...
def get_pose(image):
	...
	joints = [
		[12,0], [12,1], [12,6], [12,9], [12,13], [12,15], #connected to the Nose-Neck
		[1,4], [4,5], #connect LShoulder-LElbow-LWrist
		[0,2], [2,3], #connect RShoulder-RElbow-RWrist
		[6,7], [7,8], #connect RHip-RKnee-RAnkle
		[9,10], [10,11], #connect LHip-LKnee-LAnkle
	...
	for part in range(18):
		...
		peaks_binary = np.logical_and.reduce((map_ak>=map_left, map_ak>=map_right, map_ak>=map_up, map_ak>=map_down, map_ak > param_['thre1']))
	#    peaks_binary = T.eq(
	#    peaks = zip(T.nonzero(peaks_binary)[0],T.nonzero(peaks_binary)[0])
		...
		peaks_with_score_and_id = [peaks_with_score[i] + (some_id[i],) for i in range(len(some_id))]
			...
			for i in range(nA):
				for j in range(nB):
					...
					if norm != 0:
						...
						score_with_dist_prior = sum(score_midpts)/len(score_midpts) + min(0.5*oriImg.shape[0]-1, 0)
...
# test_image = '/Users/akbirkhan/Desktop/image1.jpeg'
# image = cv2.imread(test_image) # B,G,R order
# print(get_pose(image))
